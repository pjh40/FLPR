...
#define INGEST_DEF(term) std::optional<term> term::ingest(st_ref root)

#define ROOT_TAG_IS(X) (FLPR::Syntax_Tags::X == root->syntag)

#define EXPECT_NAME assert(FLPR::Syntax_Tags::is_name(root->syntag))
...
#define EXPECT_OPTIONAL(LHS, RHS)                                              \
  {                                                                            \
    auto tmp_optional = RHS;                                                   \
    assert(tmp_optional.has_value());                                          \
    LHS = std::move(*tmp_optional);                                            \
  }
...
INGEST_DEF(Type_Declaration_Stmt) {
  if (ROOT_TAG_IS(SG_TYPE_DECLARATION_STMT) && root.has_down()) {
    ...
    root.down();
    EXPECT_OPTIONAL(ast.type_decl_attr_seq, Type_Decl_Attr_Seq::ingest(root));
    root.next();
